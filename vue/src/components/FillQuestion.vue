<template>
  <div>
    ...
    <div style="margin: 10px 10px">
      ...
      <el-button type="primary" @click="addFill">新增</el-button>
    </div>
...
export default {
  ...
  methods:{
    ...
    handleDelete(questionId){   //根据id删除题目
      request.delete("/api/fillQuestion/deleteFill/"+questionId).
      then(resp=>{
        if(resp.code==='0'){
          this.$message({
            type:"success",
    ...
    load(){     //从数据库中查表并且显示在表格中
      request.get("api/fillQuestion/queryFill",{
        params:{
    ...
    save(){   //保存修改题目（新增和修改）
      if(this.form.questionId){
        request.put("/api/fillQuestion/updateFill",this.form).
        then(resp=>{
          if(resp.code==='0'){
            this.$message({
              type:"success",
        ...
        request.post("api/fillQuestion/addFill",this.form).
        then(resp=>{
          if(resp.code==='0'){
            this.$message({
              type:"success",
