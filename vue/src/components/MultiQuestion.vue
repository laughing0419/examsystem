...
<!--    搜索框-->
    <div style="margin: 10px 10px">
      ...
      <el-button type="primary" @click="addMulti">新增</el-button>
    </div>
...
<!--分页      -->
    ...
    <!--      弹出对话框-->
    <el-dialog title="修改题目信息" :visible.sync="dialogFormVisible" width="60%">
...
export default {
  ...
  methods:{
    ...
    handleDelete(questionId){   //根据id删除题目
      request.delete("/api/multiQuestion/deleteMulti/"+questionId).
      then(resp=> {
        if(resp.code==='0'){
          this.$message({
            type:"success",
    ...
    save(){   //保存修改题目（新增和修改）
        if(this.form.questionId){
         request.put("api/multiQuestion/updateMulti",this.form)
          .then(resp=>{
            if(resp.code==='0'){
              this.$message({
                type:"success",
          ...
          request.post("api/multiQuestion/addMulti",this.form).then(resp=>{
            if(resp.code==='0'){
              this.$message({
                type:"success",
                message:resp.msg
