          ...
          <div class="container">
            ...
            <el-form :label-positon="labelPosition" label-width="80px" :model="formLogin">
              <el-form-item label="用户名">
                <el-input v-model="formLogin.studentId" prefix-icon="el-icon-user" placeholder="请输入用户名"></el-input>
              </el-form-item>
...
import request from "../../../../vue/src/utils/request";
...
export default {
  ...
  methods:{
    //用户登录
    login(){
      if (this.formLogin.studentId!=null&&this.formLogin.pwd!=null&&this.formLogin.role!=null){
        if(this.formLogin.role==='学生'){
          console.log(this.formLogin)
          request.post("/api/login/student",this.formLogin).
          then(resp=>{
            if(resp.code==='0'){
              this.$message({
                type:'success',
        ...
        }else if(this.formLogin.role==='教师'){
          this.$message({
            type:"warning",
            message:'你无权限'
          })
